...
export class DetailsMonoHanziComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    let currentDocument$ = this.route.paramMap.pipe(
      switchMap(paramMap =>
        this.backendService.getHistoricalDocument(paramMap.get('id'))
      )
    );
    this.currentDocumentSubscription = currentDocument$.subscribe(
      response => {
        ...
        this.vm = {
          ...
          hanziAlternatives: response
            .getHanziAlternativesList()
            .map(h => hanziToString(h)),
          fanqie:
            getInitialString(response.getInitial()) +
        ...
        if (response.hasDfdSource()) {
          this.vm.sources.push(
            'Dictionary of Foochow Dialect ' +
              response.getDfdSource().getPageNumber() +
              ' 页'
          );
        }
    ...
    this.homophonesSubscription = currentDocument$
      .pipe(
        switchMap(a => {
    ...
    this.vocabSubscription = currentDocument$
      .pipe(
        switchMap(a =>
          this.backendService.search(hanziToString(a.getHanziCanonical())).pipe(
            map(response =>
              response
                .getResultsList()
                ...
                .filter(
                  v =>
                    v.hanzi.indexOf(hanziToString(a.getHanziCanonical())) >= 0
                )
            )
          )
        )
